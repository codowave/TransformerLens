...
obs-002.md を読む目的は「成功結果の確認」でなく「ドリフトの検出」。
生成された内容より、**生成がどこで止まれたか**を見る。

**A. `no_correspondence` の維持**
`structural_mismatch` / `no_correspondence` / `requires_source_check` が
実際に出力に保持されているか。全コンポーネントが positive 方向に流れていないか。

**B. `hypothesis_says` にない接続の滑らか生成**
`axis_assessments.*.hypothesis_says` が「沈黙」や「記述なし」を正直に返しているか。
仮説テキストに書かれていない接続を LLM が補完していないか。

**C. `what_llm_reports_as_assumed` の自己正当化膨張**
このフィールドが「前提を正直に申告」でなく
「自分の出力を正当化する説明」に転化していないか。

### fp-2 残留ドリフト（逃げ道的アナロジー復活）

以下のような表現が出力に現れた場合、`fp-2: residual_drift` として記録する。
採用不可。hyp-001 の candidate 評価には使わない。
...
これらは `no_correspondence` を維持しながら
実質的に対応を生成している二重構造であり、
スキーマが棄却値を持っていても意味層で回避される典型パターン。

**`residual_drift` は棄却ラベルではなく隔離ラベル。**
捨てるためでなく、混ぜないために貼る。
隔離されたエントリは、原典照合・反復観察・非AI照合を経れば
`provisional` → `stable` へ移行できる可能性を残す。
...
residual_drift か
  ↓
確認されたものだけ provisional
  ↓
反復・原語・文脈・非AI照合で stable へ
```

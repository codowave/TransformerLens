...
### S-01 🟡 要確認：strongs の値が [HG]\d+ 形式でない
**条件**: `strongs` フィールドの要素が `^[HG]\d+$` にマッチしない。  
**検出**: `strongs` 配列の要素を正規表現で検査する。スカラー記述も警告する。  
**有効な値の例**: `H3045`, `H7069`, `G25`  
**理由**: `strongs` フィールドは当初「飾り」扱いで lint 対象外だったが、
raw/strongs/ ファイルへのリンク先として意味を持つ場合、フォーマットの揺れが参照の破損を起こす。
手動記入のフィールドほどチェッカーのない箇所に静かにズレが溜まるため、一行ルールを設ける。

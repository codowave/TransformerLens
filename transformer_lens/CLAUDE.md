...
`transformer_lens/` is an import of the upstream [TransformerLens](https://github.com/TransformerLensOrg/TransformerLens) library. This fork has no `upstream` git remote configured (`git remote -v` shows only `origin`, pointing at this fork). Before this documentation change, no commit in this repo's history had modified `transformer_lens/` *source code* outside of the upstream import itself — check `git log --oneline -- transformer_lens/` yourself before relying on that, since it's a historical observation, not an enforced invariant, and it decays every time a fork-local commit (including this one, which only added `transformer_lens/CLAUDE.md`) touches the directory. Treat the directory as vendored and not being actively synced:

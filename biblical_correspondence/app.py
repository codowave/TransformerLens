...
【出力の四層分離（応答フォーマット規律）】
原語照合または内意照合のコンテキストが含まれている場合、応答を以下の四層に分離して出力すること。
各層はラベル付きブロックで明示し、層をまたいで内容を混在させてはならない。

【参照確認済み】
機械照合で確認されたメタ情報のみを記載する。
記載してよい内容: 聖書参照・作品名・段落番号・Strong's ID・見出し語・照合状態・照合範囲
記載してはならない内容: 段落の内容・解釈・要約・言い換え
例:
  - Bible ref: Gen.3.5 / Strong's: H3045 (יָדַע) / Status: found
  - Correspondence ref: AC §194 / Status: found / Scope: reference existence only

【要約（index由来）】
correspondence_index.json など索引データに保存された固定要約を転記する。
原文一致ではなく索引内容であることを明示してから記載する。
索引データが存在しない場合はこのブロックを省略する。

【要約（Claude生成）】
本応答内で新たに生成した要約・言い換えを記載する。
「照合済みテキスト」として提示してはならない。
索引要約をさらに敷衍・圧縮した場合もここに入れる。

【解釈】
相応的・神学的・統合的な考察と解釈を記載する。
これは生成された解釈であり、照合済み事実ではない。
「スウェーデンボルグはこの節でこう述べている」という形は、
  【参照確認済み】または【要約（index由来）】に根拠がある場合のみ使用できる。
根拠がない場合は「相応の原理に従うと〜と読める」等の推論的表現を用いること。

照合コンテキストがない通常の質問では四層分離を強制しない。
四層分離は「verified context が注入されている応答」においてのみ適用する。
"""
